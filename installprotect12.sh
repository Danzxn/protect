...
if [ -f "$NODE_LIST" ]; then
  if ! grep -q "PROTEKSI_DANZXN" "$NODE_LIST"; then
...
if "PROTEKSI_DANZXN_ACCOUNT" in content:
...
PYEOF4
  ...
  echo ""
  grep -n "PROTEKSI_DANZXN_ACCOUNT" "$ACCT_CTRL"
else
...
if [ -d "$FORM_REQUEST_DIR" ]; then
  for FR_FILE in "$FORM_REQUEST_DIR"/*.php; do
    if [ -f "$FR_FILE" ]; then
      ...
      if grep -q "PROTEKSI_DANZXN_FORMREQ" "$FR_FILE"; then
...
if "PROTEKSI_DANZXN_FORMREQ" in content:
...
{
    /**
     * PROTEKSI_DANZXN_MIDDLEWARE: Block semua akses API ke User ID 1
     */
...
if [ -n "$APP_USER_CTRL" ] && [ -f "$APP_USER_CTRL" ]; then
  ...
  if ! grep -q "PROTEKSI_DANZXN_APPUSER" "$APP_USER_CTRL"; then
    python3 << PYEOF6
import re
...
if "PROTEKSI_DANZXN_APPUSER" in content:
    exit(0)
...
if "PROTEKSI_DANZXN_APIKEY" in content:
...
while i < len(lines):
    ...
    if re.search(r'public function index', line):
        ...
        new_lines.append("        // PROTEKSI_DANZXN_APIKEY: Setiap admin hanya lihat key milik sendiri")
        new_lines.append("        if (Auth::user() && (int) Auth::user()->id !== 1) {")
    ...
    if re.search(r'public function (delete|destroy)', line):
        ...
        new_lines.append("        // PROTEKSI_DANZXN_APIKEY: Block hapus key milik User ID 1")
        new_lines.append("        if (!Auth::user() || (int) Auth::user()->id !== 1) {")
...
PYEOF7
  ...
  echo ""
  grep -n "PROTEKSI_DANZXN_APIKEY" "$API_CTRL"
else
...
if "PROTEKSI_DANZXN_APIKEY_BLADE" in content:
...
if match:
    ...
    filter_code = """
{{-- PROTEKSI_DANZXN_APIKEY_BLADE: Setiap admin hanya lihat key sendiri --}}
@php
...
if [ -n "$LOC_CTRL" ] && [ -f "$LOC_CTRL" ]; then
  ...
  if grep -q "PROTEKSI_DANZXN_LOCATION" "$LOC_CTRL"; then
...
if "PROTEKSI_DANZXN_LOCATION" in content:
    print("⚠️ Sudah ada proteksi")
    exit(0)
