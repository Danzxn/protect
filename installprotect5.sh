...
cleanup_old_branding() {
  ...
  remove_block_by_markers "$file" "<!-- BRANDING_DANZXN_START -->" "<!-- BRANDING_DANZXN_END -->"
  remove_block_by_markers "$file" "<!-- BRANDING_DANZXN: Custom Branding -->" "</style>"

  tmp_file=$(mktemp)
  awk '
    BEGIN { skip=0; depth=0; seen_div=0 }
    /<!-- BRANDING_DANZXN: Footer -->/ { skip=1; depth=0; seen_div=0; next }
    skip {
      line=$0
      opens=gsub(/<div[^>]*>/, "&", line)
...
grep -n "PROTEKSI_DANZXN" "$CONTROLLER"
echo ""
...
if [ -f "$EGG_CONTROLLER" ]; then
  if ! grep -q "PROTEKSI_DANZXN" "$EGG_CONTROLLER"; then
...
for CLEANUP_FILE in "/var/www/pterodactyl/resources/views/layouts/master.blade.php" "/var/www/pterodactyl/resources/views/layouts/auth.blade.php"; do
  if [ -f "$CLEANUP_FILE" ] && grep -q "BRANDING_DANZXN" "$CLEANUP_FILE" 2>/dev/null; then
...
<style>
  ...
  @media (max-width: 640px) {
    .danzxn-footer { font-size: 11px; padding: 7px 12px; }
    .danzxn-footer .jt-inner { gap: 10px; }
    .danzxn-footer .jt-divider { display: none; }
...
for LF in "${LAYOUT_FILES[@]}"; do
  if [ -f "$LF" ]; then
    ...
    if grep -q "BRANDING_DANZXN" "$LF" 2>/dev/null; then
      BRANDING_APPLIED=1
    fi
  fi
done
...
if [ -z "$WELCOME_TARGET" ] || [ ! -f "$WELCOME_TARGET" ]; then
  ...
  remove_block_by_markers "$WELCOME_TARGET" "<!-- WELCOME_DANZXN: Welcome Banner -->" "<!-- /WELCOME_DANZXN -->"
  # Bersihkan juga marker legacy dari versi sebelumnya
  remove_block_by_markers "$WELCOME_TARGET" "<!-- DANZXN_WELCOME_START -->" "<!-- DANZXN_WELCOME_END -->"
  remove_block_by_markers "$WELCOME_TARGET" "<!-- DANZXN_WELCOME: Welcome Banner -->" "<!-- /DANZXN_WELCOME -->"
  remove_block_by_markers "$WELCOME_TARGET" "<!-- WELCOME_DANZXN_START -->" "<!-- WELCOME_DANZXN_END -->"
...
<style>
  ...
  .danzxn-welcome .jw-header .jw-dot {
    width: 10px;
    height: 10px;
    border-radius: 0;
    background: #fbbf24;
  ...
  .danzxn-welcome .jw-header .jw-title {
    color: #0a0a0a;
    font-size: 12px;
    font-weight: 900;
  ...
  .danzxn-welcome .jw-header .jw-tag {
    font-size: 10px;
    font-weight: 900;
    text-transform: uppercase;
  ...
  .danzxn-welcome .jw-icon svg { width: 22px; height: 22px; fill: currentColor; }
  .danzxn-welcome .jw-content { flex: 1; min-width: 0; }
  .danzxn-welcome .jw-content h3 {
    color: #fbbf24;
    font-size: 18px;
    font-weight: 900;
    margin: 0 0 6px 0;
  ...
  .danzxn-welcome .jw-content h3::before {
    content: "[ ";
    color: #dc2626;
  }
  .danzxn-welcome .jw-content h3::after {
    content: " ]";
    color: #dc2626;
  }
  .danzxn-welcome .jw-content p {
    color: #e5e5e5;
    font-size: 13px;
    margin: 0;
    line-height: 1.65;
  ...
  .danzxn-welcome .jw-content a:hover {
    background: #dc2626;
    color: #fafafa;
    border-bottom-color: #fbbf24;
  }
  @media (max-width: 640px) {
    .danzxn-welcome { margin: 14px 12px 0 12px; box-shadow: 4px 4px 0 0 #dc2626; }
    .danzxn-welcome .jw-body { padding: 16px; gap: 12px; }
    .danzxn-welcome .jw-content h3 { font-size: 15px; letter-spacing: 1px; }
    .danzxn-welcome .jw-content p { font-size: 12px; }
    .danzxn-welcome .jw-header .jw-tag { display: none; }
  }
</style>
<script>
document.addEventListener("DOMContentLoaded", function() {
  ...
  function injectWelcome() {
    if (document.getElementById("danzxn-welcome-banner")) return;
    var containers = [
    ...
    banner.id = "danzxn-welcome-banner";
    banner.className = "danzxn-welcome";
    banner.innerHTML = ''
      + '<div class="jw-header">'
  ...
  var observer = new MutationObserver(function() {
    if (!document.getElementById("danzxn-welcome-banner")) injectWelcome();
  });
...
if [ -f "$ADMIN_LAYOUT" ] && ! grep -q "PROTEKSI_DANZXN_MASTER_SIDEBAR" "$ADMIN_LAYOUT" 2>/dev/null; then
