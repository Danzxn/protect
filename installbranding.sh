...
<style>
  /* ===== Danzxn Store Branding - Cyber Red Neo-Brutalist ===== */
  .danzxn-footer {
    position: fixed;
    bottom: 0;
    left: 0;
...
<div class="danzxn-footer">
  <div class="jt-inner">
    ...
    <span class="jt-text">PANEL BY <a href="https://t.me/danangvalentp" target="_blank">DANZXN STORE</a></span>
...
for LF in "${LAYOUT_FILES[@]}"; do
  if [ -f "$LF" ]; then
    if grep -q "<title>" "$LF" && ! grep -q "Danzxn Store" "$LF"; then
...
echo ""
echo "==========================================="
echo "✅ Branding Danzxn Store terpasang!"
echo "==========================================="
