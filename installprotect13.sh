...
if [ ! -f "$SIDEBAR_FILE" ]; then
    ...
    if grep -q "PROTEKSI_DANZXN_APPAPI_MENU" "$SIDEBAR_FILE"; then
        ...
        if [ -n "$LINE_NUM" ]; then
            ...
            sed -i "${END_LINE}a\\{{-- END PROTEKSI_DANZXN_APPAPI_MENU --}}" "$SIDEBAR_FILE"
            sed -i "${END_LINE}a\\@endif" "$SIDEBAR_FILE"

            # Insert @if sebelum START_LINE
            sed -i "$((START_LINE))i\\@if(Auth::user()->id === 1)" "$SIDEBAR_FILE"
            sed -i "$((START_LINE))i\\{{-- PROTEKSI_DANZXN_APPAPI_MENU: Sembunyikan untuk non-ID 1 --}}" "$SIDEBAR_FILE"
...
if [ ! -f "$API_CONTROLLER" ]; then
    ...
    if grep -q "PROTEKSI_DANZXN_APPAPI_BLOCK" "$API_CONTROLLER"; then
        echo "⚠️ Proteksi sudah ada, skip..."
    else
        ...
        if [ -n "$INDEX_LINE" ]; then
            ...
            sed -i "${BRACE_LINE}a\\        // PROTEKSI_DANZXN_APPAPI_BLOCK: Block akses untuk non-ID 1" "$API_CONTROLLER"
        ...
        if [ -n "$STORE_LINE" ]; then
            ...
            sed -i "${BRACE_LINE}a\\        // PROTEKSI_DANZXN_APPAPI_BLOCK" "$API_CONTROLLER"
            sed -i "$((BRACE_LINE + 1))a\\        if (\\\\Auth::user()->id !== 1) { abort(403, 'Akses Application API tidak diizinkan.'); }" "$API_CONTROLLER"
...
if [ -z "$API_USER_CONTROLLER" ] || [ ! -f "$API_USER_CONTROLLER" ]; then
    ...
    if grep -q "PROTEKSI_DANZXN_API_ROOTADMIN" "$API_USER_CONTROLLER"; then
        echo "⚠️ Proteksi sudah ada, skip..."
    else
        ...
        if [ -n "$STORE_LINE" ]; then
            ...
            sed -i "${BRACE_LINE}a\\        // PROTEKSI_DANZXN_API_ROOTADMIN: Block non-ID 1 dari set root_admin via API" "$API_USER_CONTROLLER"
        ...
        if [ -n "$UPDATE_LINE" ]; then
            ...
            sed -i "${BRACE_LINE}a\\        // PROTEKSI_DANZXN_API_ROOTADMIN: Block non-ID 1 dari ubah root_admin via API" "$API_USER_CONTROLLER"
        ...
        if [ -n "$DELETE_LINE" ]; then
            ...
            sed -i "${BRACE_LINE}a\\        // PROTEKSI_DANZXN_API_ROOTADMIN: Block non-ID 1 dari hapus user via API" "$API_USER_CONTROLLER"
